"""
rate_limiter.py — Groq API Rate Limiter for Phase 2
...
Rate Limits (llama-3.3-70b-versatile free tier):
  - 30 requests/min
  - 1,000 requests/day
  - 12,000 tokens/min   ← tightest constraint
  - 100,000 tokens/day
"""
...
# ---------------------------------------------------------------------------
MIN_CALL_SPACING_SECONDS = 60       # ≥ 60s between calls (12K TPM safety)
MAX_TOKENS_PER_CALL = 10_000        # Hard cap per single call
DAILY_TOKEN_BUDGET = 90_000         # Abort threshold (buffer before 100K)
MAX_RETRIES = 3                     # Max retries on 429
INITIAL_BACKOFF_SECONDS = 30        # First retry wait
...
class GroqRateLimiter:
    ...
    def check_budget(self, estimated_input_tokens: int, estimated_output_tokens: int = 1500):
        ...
        Raises:
            ...
            ValueError: if single call exceeds per-call limit
        """
        self._reset_daily_if_needed()

        total_estimated = estimated_input_tokens + estimated_output_tokens

        if total_estimated > self.max_tokens_per_call:
            raise ValueError(
                f"Estimated call tokens ({total_estimated}) exceed per-call "
                f"limit ({self.max_tokens_per_call}). Reduce batch size."
            )
